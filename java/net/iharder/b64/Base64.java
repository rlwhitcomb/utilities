 ...
 * </p>
 * <ul>
 *  <li>v2.2.9 - Don't need or require padding if URL_SAFE is given; regularize tabs vs spaces;
 *   some other code cleanup. 17-Oct-22
 ...
 *   15-Mar-2019
 *  <li>v2.2.3 - Fix Javadoc warnings found by Java 8. 23-Feb-2016
 *  <li>v2.2.3 - Added some more convenience methods for encoding and decoding.
 *   Done by Roger.Whitcomb@ingres.com; finished changes on 23-Dec-2008.
...
/* ********  D E T E R M I N E   W H I C H   A L H A B E T  ******** */
     ...
     * @param options The set of options we're using.
     * @return The correct alphabet for the options specified.
     */
     ...
     * no guarantee as to which one will be picked.
     * @param options The set of options we're using.
     * @return The correct decodabet for the options specified.
     */
...
/* ********  E N C O D I N G   M E T H O D S  ******** */
     ...
     * @param options The various options for the encoding.
     ...
     * @param options The various options for the encoding.
     * @return the number of chars added to destination (usually 4,
     * but could be less if padding is left off for URL_SAFE use).
     * @since 1.3
     */
     ...
     byte[] destination, int destOffset, int options )
    {
        ...
        boolean urlSafe = (options & URL_SAFE) == URL_SAFE;
    ...
    /**
     * Turn the given bytes from an array back into a {@link String} object.
     * <p> Our {@link #PREFERRED_ENCODING} is used, but if this encoding
     * isn't supported, then use whatever the current system default
     * encoding is in effect.
     * <p> Only the portion of the byte array starting at <code>offset</code>
     * and going for <code>len</code> bytes will be used.  This better be
     * starting at a character boundary, .....
     *
     * @param   bytes   array of bytes (hopefully) in our <code>PREFERRED_ENCODING</code>
     * @param   offset  starting position in the array to be the encoding
     * @param   len     number of bytes to use in the conversion
     * @return          <code>String</code> object constructed from the byte array
     * @since   2.2.3
     */
    public static String bytesToString( byte[] bytes, int offset, int len )
    {
        try
        {
            return new String( bytes, offset, len, PREFERRED_ENCODING );
        }   // end try
    ...
    /**
     * Turn the given array of bytes back into a {@link String} object.
     * <p> Our {@link #PREFERRED_ENCODING} is used, but if this encoding
     * isn't supported, then use whatever the current system default
     * encoding is in effect.
     *
    ...
    /**
     * Get the bytes of a string using our {@link #PREFERRED_ENCODING}.
     * <p> If the <code>PREFERRED_ENCODING</code> doesn't work, then
     * use whatever the current default encoding is.
     *
     * @param   s    source string
     * @return       array of bytes derived from the string encoding
     * @since   2.2.3
     */
    ...
    /**
     * Encodes a String into Base64 notation using UTF-8 encoding
     * Does not GZip-compress data.
     *
     * @param source The data to convert
     * @return The Base64 encoded version.
    ...
    public static String encodeBytes( byte[] source, int off, int len, int options )
    {
        ...
        else
        {
            ...
            int lineLength = 0;
            int num = 0;
            for( ; d < len2; d+=3, e+=num )
            {
                num = encode3to4( source, d+off, 3, outBuff, e, options );

                lineLength += num;
                if( breakLines && lineLength == MAX_LINE_LENGTH )
                {
                    outBuff[e+num] = NEW_LINE;
                    e++;
                    lineLength = 0;
                }   // end if: end of line
            ...
            if( d < len )
            {
                num = encode3to4( source, d+off, len - d, outBuff, e, options );
                e += num;
...
/* ********  D E C O D I N G   M E T H O D S  ******** */
    ...
    public static byte[] decode( byte[] source, int off, int len, int options )
    {
        ...
        if( b4Posn > 0 )
        {
            ...
            {
                caughtException = new java.io.IOException( "Improperly padded Base64 input." );
                return null;
            }
        }
    ...
    {
        return bytesToString( decode( s, NO_OPTIONS ) );
    }
    ...
    public static byte[] decodeFromFile( String filename, int options )
    {
        ...
        try
        {
            ...
            {
                caughtException = new java.io.IOException( "File is too big for this convenience method (" + file.length() + " bytes)." );
                return null;
    ...
    public static class InputStream extends java.io.FilterInputStream
    {
        ...
        private boolean urlSafe;        // URL_SAFE means don't do padding
        ...
        public InputStream( java.io.InputStream in, int options )
        {
            ...
            this.urlSafe      = (options & URL_SAFE) == URL_SAFE;
        ...
        public int read() throws java.io.IOException
        {
            ...
            if( position < 0 )
            {
                if( encode )
                {
                    ...
                    if( numBinaryBytes > 0 )
                    {
                        numSigBytes = encode3to4( b3, 0, numBinaryBytes, buffer, 0, options );
                        position = 0;
                    }   // end if: got data
                    else
                    {
    ...
    public static class OutputStream extends java.io.FilterOutputStream
    {
         ...
         * @throws java.io.IOException for write errors.
         */
